...
const viewUserProfile = async (req, res) => {
  try {
    ...
    let { email, given_name, family_name, picture } = req.body;
    console.log(given_name, family_name, email, picture);
    let foundUser = await userModel.findOne({ email });
    if (foundUser) {
      const token = jwt.sign({ id: foundUser._id }, "secret_key", {
        expiresIn: "30d",
      });
      res.json({ message: "User found and logged in", foundUser, token });
    } else {
...
const signUp = async (req, res) => {
  try {
    ...
    console.log(error); //abortEarly false let's the validate method continue to validate all fields even after facing a validation error. normally it stops at the first error.
    if (error) {
      ...
      if (!foundUser) {
        ...
        //hash takes a callback as its third parameter which will be called when the hash is completed. bcrypt. hashSync runs the hash, waits for it to complete and returns the hashed value. In other words "hash" is asynchronous and hashSync is synschronous.
        let addedUser = await userModel.create({
          ...req.body,
...
const signIn = async (req, res) => {
  console.log("DB received the request");
  ...
  if (error) {
    ...
    //[x] console.log("Found user"+foundUser);
    if (!foundUser) {
      res
        .status(404)
